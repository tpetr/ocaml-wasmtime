...
let resolve_path p =
  if Filename.is_relative p then
    (* Inside dune, cwd is the build dir; use DUNE_SOURCEROOT to resolve
       paths relative to the project root where the user invoked dune. *)
    let root = match Sys.getenv_opt "DUNE_SOURCEROOT" with
      | Some r -> r
      | None -> Sys.getcwd ()
    in
    Filename.concat root p
  else p
...
let wasmtime_flags () =
  ...
  in
  (* Priority: LIBWASMTIME env > Homebrew > OPAM_SWITCH_PREFIX *)
  match Sys.getenv_opt "LIBWASMTIME" with
  | Some lib_dir -> config ~lib_dir
  | None ->
    match homebrew_prefix () with
    | Some prefix -> config ~lib_dir:prefix
    | None ->
